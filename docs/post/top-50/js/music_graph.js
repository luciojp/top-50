...
d3.json('meutop50ok.json', function (error, graph) {
...
function genreX(n) {
    ...
    if (genres.includes('black metal') || genres.includes('death metal')) {
        return width / 4 * 3;
...
function genreY(n) {
    const genres = n.genres.join('-');
    if (genres.length === 0 && !genres.includes('black metal') && !genres.includes('death metal') && genres.includes('metal')) {
        return height / 4;
    } else {
        return height / 2;
    }
